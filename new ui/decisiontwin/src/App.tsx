...
import BiasToDollar from "./components/BiasToDollar";
import ModelComparison from "./components/ModelComparison";
import PersonaExplorer from "./components/PersonaExplorer";
import ComplianceWarRoom from "./components/ComplianceWarRoom";
import AuditGenerator from "./components/AuditGenerator";
import GovernanceMap from "./components/GovernanceMap";
...
import { 
  ...
  ArrowRight, SearchCode, BrainCircuit, Globe, Info
} from "lucide-react";
...
export default function App() {
  ...
  const NAV_ITEMS = [
    ...
    { id: "policy", label: "Policy Lab", icon: <Sliders className="w-4 h-4" /> },
    { id: "dollar", label: "Bias-to-Dollar Translator", icon: <DollarSign className="w-4 h-4" /> },
    { id: "comparison", label: "Model Comparison Lab", icon: <Layers className="w-4 h-4" /> },
    { id: "explorer", label: "Persona Explorer", icon: <SearchCode className="w-4 h-4" /> },
    { id: "compliance", label: "Compliance War Room", icon: <ShieldCheck className="w-4 h-4" /> },
    { id: "audit", label: "Audit Report Generator", icon: <FileText className="w-4 h-4" /> },
    { id: "map", label: "Real-Time Gov Map", icon: <Map className="w-4 h-4" /> },
    { id: "boardroom", label: "Boardroom Presentation", icon: <Presentation className="w-4 h-4" /> }
  ];
    ...
    <div className={`min-h-screen p-bg-main p-text-main flex flex-col font-sans relative transition-colors duration-500 ${theme === "cream" ? "theme-cream" : "theme-mono"}`} id="applet-main-container">
      ...
      <div className="flex-grow flex flex-col lg:flex-row w-full max-w-[1440px] mx-auto px-6 py-6 gap-6 relative z-10">
        ...
        <main className="flex-grow min-w-0" id="workbench-core">
          ...
          {activeTab === "policy" && <PolicyLab />}
          {activeTab === "dollar" && <BiasToDollar />}
          {activeTab === "comparison" && <ModelComparison />}
          {activeTab === "explorer" && <PersonaExplorer />}
          {activeTab === "compliance" && <ComplianceWarRoom />}
          {activeTab === "audit" && <AuditGenerator />}
          {activeTab === "map" && <GovernanceMap />}
          {activeTab === "boardroom" && (
            <ComingSoonOverlay>
