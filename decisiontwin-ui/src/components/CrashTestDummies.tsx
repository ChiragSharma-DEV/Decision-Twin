...
export default function CrashTestDummies({ onSelectPersona, onNavigateToTab }: CrashTestDummiesProps) {
  const { personas, loading, error, runCrashTestFlow, session, overrideDecisionFlow } = useDecisionTwinStore();
  const [selectedPersona, setSelectedPersona] = useState<Persona | null>(null);
  const [overrideYear, setOverrideYear] = useState<number>(1);
  ...
  const handleApplyOverride = async (newDecision: number) => {
    if (!selectedPersona || !session) return;
    setIsOverriding(true);
    setOverrideSuccess(null);
    try {
      const personaIndex = personas.findIndex(p => p.id === selectedPersona.id);
      const targetIndex = session.row_count + (personaIndex >= 0 ? personaIndex : 0);
      
      await overrideDecisionFlow({
        year: overrideYear,
        row_index: targetIndex,
        new_decision: newDecision
      });
      setOverrideSuccess(`Successfully overridden to ${newDecision === 1 ? 'Approved' : 'Rejected'}! Simulation updated.`);
        ...
        <div className="lg:col-span-4">
          <div className="p-6 rounded border p-border p-bg-card space-y-6 sticky top-24 shadow-sm">
            ...
            <div className="space-y-4 border-t p-border pt-4">
              ...
              <div className="p-4 p-bg-secondary rounded border p-border space-y-3.5 shadow-inner">
                <div className="flex justify-between items-center">
                  <span className="text-[9px] font-mono p-text-accent font-bold uppercase tracking-wider block">HITL INTERVENTION PANEL</span>
                  <span className="text-[8px] font-mono p-text-muted uppercase">Manual Decision Override</span>
                </div>
                
                <div className="flex items-center gap-2.5">
                  <label className="text-[9px] font-mono p-text-muted uppercase tracking-wider font-bold shrink-0">SIMULATION YEAR:</label>
                  <select
                    value={overrideYear}
                    onChange={(e) => setOverrideYear(Number(e.target.value))}
                    className="flex-grow p-bg-secondary border p-border rounded px-2.5 py-1 text-xs p-text-main focus:outline-none cursor-pointer font-mono"
                  >
                    {Array.from({ length: session?.years_simulated ?? 10 }, (_, i) => i + 1).map((y) => (
                      <option key={y} value={y}>Year {y} ({2025 + y})</option>
                    ))}
                  </select>
                </div>
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => handleApplyOverride(1)}
                    disabled={isOverriding}
                    className="flex-1 py-1.5 rounded font-mono text-[10px] font-bold uppercase tracking-wider bg-emerald-600 hover:bg-emerald-700 text-white cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed shadow-sm transition-all text-center"
                  >
                    {isOverriding ? "Applying..." : "Approve Override"}
                  </button>
                  <button
                    onClick={() => handleApplyOverride(0)}
                    disabled={isOverriding}
                    className="flex-1 py-1.5 rounded font-mono text-[10px] font-bold uppercase tracking-wider bg-rose-600 hover:bg-rose-700 text-white cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed shadow-sm transition-all text-center"
                  >
                    {isOverriding ? "Applying..." : "Reject Override"}
                  </button>
                </div>

                {overrideSuccess && (
                  <p className="text-[9px] font-mono p-text-success text-center animate-fade-in font-bold leading-normal mt-1">{overrideSuccess}</p>
                )}
              </div>
            </div>
