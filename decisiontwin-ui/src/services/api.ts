...
export async function runDoppelganger(): Promise<DoppelgangerResponse> {
  const res = await fetch(`${FASTAPI_BASE_URL}/run-doppelganger`, {
    method: "POST",
  });
  return parseJson<DoppelgangerResponse>(res);
}
...
export async function overrideDecision(
  ...
  const res = await fetch(`${FASTAPI_BASE_URL}/override-decision`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });
...
export async function generateDetailedReport(): Promise<{ status: string; report: string }> {
  const res = await fetch(`${FASTAPI_BASE_URL}/generate-detailed-report`, {
    method: "POST",
  });
  return parseJson<{ status: string; report: string }>(res);
}
