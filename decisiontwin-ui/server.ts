...
// Priority: Vertex AI (GCP ADC) → API Key fallback → offline mode
...
const USE_VERTEX = process.env.GOOGLE_GENAI_USE_VERTEX?.toLowerCase() === "true"
  || Boolean(GCP_PROJECT);
...
if (USE_VERTEX && GCP_PROJECT) {
  ...
  try {
    ...
    console.log(
      `DecisionTwin Backend: Gemini via Vertex AI (${GEMINI_MODEL}) — project=${GCP_PROJECT}, location=${GCP_LOCATION}`
    );
  } catch (error) {
    console.error("DecisionTwin Backend: Vertex AI init failed, trying API key fallback.", error);
  }
}

if (!ai && api_key && !PLACEHOLDER_KEYS.has(api_key)) {
  // ── API Key fallback (for local dev / non-GCP environments) ────────────
  try {
    ai = new GoogleGenAI({ apiKey: api_key });
    geminiBackend = "apikey";
    console.log(
      `DecisionTwin Backend: Gemini via API Key (${GEMINI_MODEL}).`
    );
  } catch (error) {
    console.error("DecisionTwin Backend: Error initializing Gemini with API key.", error);
  }
}

if (!ai) {
  console.log(
    "DecisionTwin Backend: No Gemini credentials found. Set GOOGLE_CLOUD_PROJECT for Vertex AI or GEMINI_API_KEY for API key mode."
  );
}
...
app.get("/api/gemini-status", (_req: Request, res: Response) => {
  res.json({ live: Boolean(ai), model: GEMINI_MODEL, backend: geminiBackend });
});
